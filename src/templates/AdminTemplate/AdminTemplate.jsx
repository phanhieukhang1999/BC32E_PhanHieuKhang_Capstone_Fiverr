    ...
    {!_.isEmpty(userLogin) ? <Fragment>
      <div style={{ display: 'flex', flexDirection: 'row-reverse', alignItems: 'center' }}>
        ...
        <button style={{ display: 'flex' }} onClick={() => {
          navigate(`/profile/${userLogin.user.name}`)
        }}>
          <span className="text-white">Hello ! {userLogin.user.name}</span> <div style={{ width: 50, height: 50, display: 'flex', justifyContent: 'center', alignItems: 'center' }} className="text-2xl ml-5 rounded-full bg-red-200">{userLogin.user.name.substr(0, 1)}</div>
        </button>
